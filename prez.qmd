...
$$
\small
\int_{t^n}^{t^{n+1}} \int_{\Omega_p} \frac{\partial T_p}{\partial t} \, dV dt = \int_{t^n}^{t^{n+1}} \left[ \color{blue}{\int_{\partial\Omega_p \setminus \Gamma} k_p \nabla T_p \cdot \mathbf{n}_p \, dS} + \color{red}{\int_{\Gamma} k_p \nabla T_p \cdot \mathbf{n}_p \, dS} + \int_{\Omega_p} S_p \, dV \right] dt
$$
...
\begin{algorithm}[H]
  \caption{\large Newton Iteration for Interface Position}
    \begin{algorithmic}[1]
      \State $k \gets 0$
      \State Initialize: $x^{n+1,0} \gets x^n$ or $x^n + \int_{t^n}^{t^{n+1}} v(t)\,dt$
      \Repeat
        \State Solve: $\partial_t T_p^k - \alpha_p \Delta T_p^k = 0$ in $\Omega_p^k(t)$
        \State Compute mismatch: $r = F(x^{n+1,k}) = \rho L v - [[q\cdot n]]$
          \If{$|r| < \epsilon$}
            \State Converged
          \Else
            \State Update: $x^{n+1,k+1} = x^{n+1,k} - (\partial_x F(x^{n+1,k}))^{-1} r$
            \State $k \gets k+1$
          \EndIf
        \Until{converged}
    \end{algorithmic}
