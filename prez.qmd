...
$$
\small
\int_{t^n}^{t^{n+1}} \int_{\Omega_p} \frac{\partial T_p}{\partial t} \, dV dt = \int_{t^n}^{t^{n+1}} \left[ \underbrace{\int_{\partial\Omega_p \setminus \Gamma} k_p \nabla T_p \cdot \mathbf{n}_p \, dS}_{\text{Regular Flux}} + \underbrace{\int_{\Gamma} k_p \nabla T_p \cdot \mathbf{n}_p \, dS}_{\text{Interface Flux}} + \int_{\Omega_p} S_p \, dV \right] dt
$$
...
\begin{algorithm}[H]
  \caption{\large Newton Iteration for Interface Position}
    \begin{algorithmic}[1]
      \State $k \gets 0$
      \State Initialize: $s^{n+1,k} \gets s^n$ or $s^n + \int_{t^n}^{t^{n+1}} v(t)\,dt$
      \Repeat
        \State Solve: $\partial_t T_p^k - \alpha_p \Delta T_p^k = 0$ in $\Omega_p^k(t)$
        \State Compute mismatch: $r = F(s^{n+1,k},T) = \rho L v - [[q\cdot n]]$
          \If{$|r| < \epsilon$}
            \State Converged
          \Else
            \State Update: $s^{n+1,k+1} = s^{n+1,k} - (\partial_s F(s^{n+1,k}))^{-1} r$
            \State $k \gets k+1$
          \EndIf
        \Until{converged}
    \end{algorithmic}
...
- **Fresh Cells**: Empty cells at $t^n$ but partially filled at $t^{n+1}$
- **Dead Cells**: Partially filled cells at $t^n$ but empty at $t^{n+1}$

- **Handling**: 
  - Still solve for temperature in those cells
  - Find the temperature to ensure conservation
...
- **Residual**: $F(\mathbf{s}_\Gamma^{n+1,(k)}) = \rho L v - ⟦k \nabla T \cdot \mathbf{n}⟧$
