...
$$
\begin{aligned}
\frac{\partial T_p}{\partial t} - \nabla \cdot (k_p \nabla T_p) &= S_p \quad \text{in } \Omega_p, \quad p=\text{liquid, solid} \\
\end{aligned}
$$
...
- $\theta$ is a parameter between 0 and 1
...
$$
V_p(T_p^{n+1} - T_p^n) = \Delta t \left[ \sum_{f \in \text{reg}} k_p A_f \nabla_f(\tilde{T}_p) + F_p^{\Gamma} + S_p V_p \right]
$$

Where $F_p^{\Gamma}$ is the interface flux in phase $p$.
...
\end{aligned}
$$
$$\alpha_s T_s^\Gamma - \alpha_l T_l^\Gamma = g(\mathbf{x})$$
...
$\Omega_s$ is a disk with radius $R$. $\Omega_l$ is the complement of $\Omega_s$ in the unit square.

$$
\begin{aligned}
\frac{\partial T_p}{\partial t} - \nabla \cdot (k_p \nabla T_p) &= 0 \quad \text{in } \Omega_p \\
T_s - \alpha_l T_l &= 0 \quad \text{on } \Gamma, \\
...
\small
\begin{aligned}
T_s(r,t) &= \frac{4 T_{s,0} k_s k_l^2 \alpha_l}{\pi^2 R} \int_0^{\infty} \exp{(-k_s u^2 t)} \frac{J_0(ur) J_1(uR)}{u^2(\Phi^2(u) + \Psi^2(u))} du \\
T_l(r,t) &= \frac{2 T_{l,0} k_s \sqrt{k_l} \alpha_l}{\pi} \int_0^{\infty} \exp{(-k_s u^2 t)} \frac{J_1(uR)(J_0(k ur)\Phi(u) - Y_0(k ur)\Psi(u))}{u(\Phi^2(u) + \Psi^2(u))} du
\end{aligned}
$$
...
![Convergence Results](images/heat_2d_2ph_henry_end.png){width=75%}
...
- Preserves conservation across time steps especially for discrete events (fresh/dead cells)
...
$$
\rho L \frac{\partial s}{\partial t} - ⟦\mathbf{q}\cdot\mathbf{n}⟧ = 0 \leftrightarrow F(s,T) = 0
$$
...
- **Stefan Condition as Root-Finding Problem**: 
  $$\rho\,L\,\dot{s} - ⟦\mathbf{q}\cdot\mathbf{n}⟧ = 0 \quad \Rightarrow \quad F(\mathbf{x}_\Gamma) = 0$$

- **Newton Iteration Method**:
  $$\mathbf{x}_\Gamma^{n+1,(k+1)} = \mathbf{x}_\Gamma^{n+1,(k)} - \left(\frac{\partial F}{\partial \mathbf{x}_\Gamma}\right)^{-1}F(\mathbf{x}_\Gamma^{n+1,(k)})$$

::: {layout-ncol=2}
![](images/newton_update.svg){width=100%, fig-align="center"}

- Iterate until convergence: $\|F(\mathbf{x}_\Gamma^{n+1,(k)})\| < \epsilon$
- Each iteration requires solving temperature field
:::
